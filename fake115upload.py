...
class  Fake115Client(object):
        ...
        def get_link(self, filename):
                try:
                        with  open(filename,'rb')  as  f:
                                ...
                                link=  filename+'|'+str(os.path.getsize(filename))+'|'+TotalHASH+'|'+BlockHASH+'\n'
                                return link
                except  Exception  as  e:
                        print(e)
                        return
        ...
        def  import_file_with_sha1(self,preid,fileid,filesize,filename):    #  pc  api
                ...
                postData  =  ('fileid=%s&filename=%s&filesize=%s&preid=%s&target=%s&userid=%s')  %  (fileid,  filename,  filesize,  preid,  target,  self.user_id)

                r  =  requests.post(url,  data=(postData.encode('utf8')),headers=self.header)
                response  =  (r.content)
