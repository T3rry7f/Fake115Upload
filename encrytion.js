...
function m115_xorinit(randkey, sk_len) {
    var length = sk_len * (sk_len - 1) var index = 0
    var xorkey = ''

    if (randkey) {
        for (var i = 0; i < sk_len; i++) {
            x = intToByte((randkey[i]) + (g_kts[index])) xorkey += String.fromCharCode(g_kts[length] ^ x) length -= sk_len index += sk_len
        }
        if (sk_len == 4) {
...
function xor115_enc(src, key) {
    ...
    for (var i = 0; i < src.length; i++) {
        ...
        secret.push((src[i] ^ key[num])) num += 1
    }
...
function genRandom(len) {
    ...
    for (i = 0; i < len; i++) {　　　　keys.push(chars.charAt(Math.floor(Math.random() * maxPos)).charCodeAt(0));　　
    }　　
    return keys;
}
...
function m115_decode(ciphertext) {
    console.log('m115_decode:') var bciphertext = forge.util.decode64(ciphertext) var block = bciphertext.length / (128) var plaintext = ''

    var index = 0

    for (var i = 1; i <= block; ++i) {
        plaintext += priv.decrypt(bciphertext.slice(index, i * 128)) index += 128
    }
...
function test() {
    ...
    var post_data = 'data=' + m115_encode('{"pickcode":"xxxxxxxxxxxxxxxxxxxxxxx"}') var options = {
        host: 'proapi.115.com',
        port: 80,
    ...
    function(res) {
        ...
        function(chunk) {
            ...
            resp = m115_decode(bodyString) console.log(JSON.parse(resp))
        });
    });
